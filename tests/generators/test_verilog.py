...
def test_todo_contains_module_name() -> None:
    m = load("uart_rx")
    v = generate_wrapper(m)
    assert "TODO: Implement uart_rx logic" in v


def test_todo_contains_parameters() -> None:
    m = load("uart_rx")
    v = generate_wrapper(m)
    assert "Parameters: DATA_WIDTH, FIFO_DEPTH, CLK_FREQ_MHZ" in v


def test_todo_contains_primary_clock() -> None:
    m = load("uart_rx")
    v = generate_wrapper(m)
    assert "Primary clock: clk" in v


def test_todo_lists_all_regs() -> None:
    m = load("uart_rx")
    v = generate_wrapper(m)
    for reg in ("rx_data", "rx_valid", "fifo_full", "fifo_data"):
        assert reg in v
